...
static int get_gd32vf103_info(struct flash_bank *bank,
		struct command_invocation *cmd);
static int gd32vf103_write_block(struct flash_bank *bank,
...
static int gd32vf103_erase(struct flash_bank *bank, unsigned int first,
		unsigned int last)
{
	struct target *target = bank->target;
	unsigned int i;
	...
	if (optiondata != 0xFFFFFFFF || (obstat & 0x2)) {
		...
		LOG_INFO(" Unlock flash success!!! Please reset platform!!\n");
		return ERROR_FAIL;
	}
	if (first && (last == (bank->num_sectors - 1)))
		return gd32vf103_mass_erase(bank);

	/* unlock flash registers */
	int retval = target_write_u32(target,
			gd32vf103_get_flash_reg(bank, FMC_KEY), UNLOCK_KEY0);
	if (retval != ERROR_OK)
		return retval;
	retval = target_write_u32(target, gd32vf103_get_flash_reg(bank, FMC_KEY),
			UNLOCK_KEY1);
	if (retval != ERROR_OK)
		return retval;
	...
	for (i = first; i <= last; i++) {
		retval = target_write_u32(target,
				gd32vf103_get_flash_reg(bank, FMC_CTL), FMC_CTL_PER);
		if (retval != ERROR_OK)
			return retval;
		retval = target_write_u32(target,
				gd32vf103_get_flash_reg(bank, FMC_ADDR),
...
static int gd32vf103_protect(struct flash_bank *bank, int set,
...
#include "../../../contrib/loaders/flash/gd32v/gd32vf103.inc"
	...
	uint32_t thisrun_bytes = fifo_end_addr - fifo_start_addr;
	thisrun_bytes -= 2; // subtract block size of 2
		...
		struct command_invocation *cmd)
{
	...
	default:
		command_print_sameline(cmd,
				"Cannot identify target as a GD32VF103 x\n");
		return ERROR_FAIL;
	}

	command_print_sameline(cmd, "%s - Rev: %s", device_str,
			rev_str ? rev_str : "unknown");
